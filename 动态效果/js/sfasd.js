...
S.UI = (function() {
	...
	function timedAction(fn, delay, max, circulation) {
		...
		if(!max || (currentAction <= max)) {
			if(max && currentAction == max && centerShow.indexOf('#countdown') != -1) {
				currentAction = 1;
			} else if(max && currentAction == max && centerShow.indexOf('#countdown') == -1) {
				currentAction = 0;
			}
			fn(currentAction);
...
S.ShapeBuilder = (function() {
	...
	function setFontSize(s) {
		//shapeContext.font = 6*hFont*s + 'px ' + fontFamily;	
	...
	return {
		...
		imageFile: function(url, callback) {
			...
			image.onerror = function() {
				callback(S.ShapeBuilder.letter('Whagt?'));
			};
		...
		render: function() {
			for(var d = 0; d < dots.length; d++) {
				if(return_array.length > 0) {
					...
					dots[d].render(return_array[j].image);
				} else {
